...
function write(type, category, message){
    return validateConfig()
        .then(getOrCreateFile)
        .then(rotateLogs)
        .then(function(){
            return writeLog(type, category,message);
        })
        .catch(function(err){
...
function validateConfig(){
    return new Promise(function(resolve) {
        ...
        if(logger.config.type.toLowerCase() !== 'file'){
        ...
        if(!logger.config.application){
            throw new Error(resources.errors.missingApplication);
        }

        if(!logger.config.machine){
...
function getOrCreateFile(){
    return Promise.resolve(getFileStats()
...
function getFileStats(){
    return fs.statAsync(logger.config.path);
}
...
function rotateLogs(stats){
    ...
    if(logDatesMatch(today, modifiedDate)){
        ...
        fs.renameAsync(logger.config.path, archivedFileName)
            .then(createEmptyFile);
    }
}
...
function writeLog(type, category, message){
    ...
    return fs.appendFileAsync(logger.config.path, entry, {});
}
...
function createEmptyFile(){
    return fs.writeFileAsync(logger.config.path, '')
        .then(function() {
            return 'success';
        });
}
