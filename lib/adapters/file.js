...
function write(type, category, message){
    ...
    return Q.nfcall(fs.appendFile, logger.config.path, entry, {});
}
...
function getFileStats(){
    return Q.nfcall(fs.stat, logger.config.path);
}
...
function rotateLogs(stats){
    ...
    if(logDatesMatch(today, modifiedDate)){
        ...
        Q.nfcall(fs.rename, logger.config.path, archivedFileName)
            .then(function() {
                return 'logrotated';
            });
    }
...
function createEmptyFile(){
    return Q.nfcall(fs.writeFile, logger.config.path, '')
        .then(function() {
            return 'success';
        });
}
...
logger.write = function(type, category, message){
    return Q.fcall(validateConfig)
        ...
        .then(function(logRotation){
            ...
            // Wait for new file creation if needed
